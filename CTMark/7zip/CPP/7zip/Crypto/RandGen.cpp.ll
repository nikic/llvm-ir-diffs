; ModuleID = '/home/npopov/repos/llvm-test-suite/CTMark/7zip/CPP/7zip/Crypto/RandGen.cpp'
source_filename = "/home/npopov/repos/llvm-test-suite/CTMark/7zip/CPP/7zip/Crypto/RandGen.cpp"
...
for.cond.cleanup:                                 ; preds = %for.cond.cleanup9
  ...
  store i8 0, ptr %_needInit, align 1
...
if.end:                                           ; preds = %if.then, %for.body
  ...
  store i64 %call5, ptr %v2, align 8
  ...
  store i32 %call6, ptr %tickCount, align 4
...
for.cond.cleanup9:                                ; preds = %for.body10
  ...
  br i1 %exitcond23.not, label %for.cond.cleanup, label %for.body
...
if.end:                                           ; preds = %if.then, %entry
  %cmp.not18 = icmp eq i32 %size, 0
  br i1 %cmp.not18, label %while.end, label %while.body.lr.ph

while.body.lr.ph:                                 ; preds = %if.end
  %_count2.i = getelementptr inbounds %"class.NCrypto::NSha1::CContextBase2", ptr %hash, i64 0, i32 1
  %1 = getelementptr inbounds [20 x i8], ptr %buff, i64 0, i64 4
  %2 = getelementptr inbounds [20 x i8], ptr %buff, i64 0, i64 8
  %3 = getelementptr inbounds [20 x i8], ptr %buff, i64 0, i64 12
  %4 = getelementptr inbounds [20 x i8], ptr %buff, i64 0, i64 16
  br label %while.body

while.body:                                       ; preds = %while.body.lr.ph, %for.cond.cleanup
  %size.addr.020 = phi i32 [ %size, %while.body.lr.ph ], [ %dec.lcssa, %for.cond.cleanup ]
  %data.addr.019 = phi ptr [ %data, %while.body.lr.ph ], [ %incdec.ptr.lcssa, %for.cond.cleanup ]
  call void @llvm.lifetime.start.p0(i64 104, ptr nonnull %hash) #9
  call void @_ZN7NCrypto5NSha112CContextBase4InitEv(ptr noundef nonnull align 8 dereferenceable(32) %hash)
  store i32 0, ptr %_count2.i, align 8
  call void @_ZN7NCrypto5NSha18CContext6UpdateEPKhm(ptr noundef nonnull align 8 dereferenceable(100) %hash, ptr noundef nonnull %this, i64 noundef 20)
  ...
  store i32 -160257071, ptr %salt, align 4
  ...
  %5 = add i32 %size.addr.020, -1
  %umin = call i32 @llvm.umin.i32(i32 %5, i32 19)
  %6 = add nuw nsw i32 %umin, 1
  %wide.trip.count = zext i32 %6 to i64
  %min.iters.check = icmp ult i32 %size.addr.020, 4
  %data.addr.01922 = ptrtoint ptr %data.addr.019 to i64
  %7 = sub i64 %data.addr.01922, %buff23
...
vector.ph:                                        ; preds = %while.body
  ...
  %ind.end = getelementptr i8, ptr %data.addr.019, i64 %n.vec
  %8 = insertelement <4 x i32> <i32 poison, i32 0, i32 0, i32 0>, i32 %size.addr.020, i64 0
  %wide.load = load <4 x i8>, ptr %buff, align 16
  store <4 x i8> %wide.load, ptr %data.addr.019, align 1
  %9 = add <4 x i32> %8, <i32 -1, i32 -1, i32 -1, i32 -1>
  %10 = icmp eq i64 %n.vec, 4
  br i1 %10, label %middle.block, label %vector.body.1

vector.body.1:                                    ; preds = %vector.ph
  %next.gep.1 = getelementptr i8, ptr %data.addr.019, i64 4
  %wide.load.1 = load <4 x i8>, ptr %1, align 4
  store <4 x i8> %wide.load.1, ptr %next.gep.1, align 1
  %11 = add <4 x i32> %8, <i32 -2, i32 -2, i32 -2, i32 -2>
  %12 = icmp eq i64 %n.vec, 8
  br i1 %12, label %middle.block, label %vector.body.2

vector.body.2:                                    ; preds = %vector.body.1
  %next.gep.2 = getelementptr i8, ptr %data.addr.019, i64 8
  %wide.load.2 = load <4 x i8>, ptr %2, align 8
  store <4 x i8> %wide.load.2, ptr %next.gep.2, align 1
  %13 = add <4 x i32> %8, <i32 -3, i32 -3, i32 -3, i32 -3>
  %14 = icmp eq i64 %n.vec, 12
  br i1 %14, label %middle.block, label %vector.body.3

vector.body.3:                                    ; preds = %vector.body.2
  %next.gep.3 = getelementptr i8, ptr %data.addr.019, i64 12
  %wide.load.3 = load <4 x i8>, ptr %3, align 4
  store <4 x i8> %wide.load.3, ptr %next.gep.3, align 1
  %15 = add <4 x i32> %8, <i32 -4, i32 -4, i32 -4, i32 -4>
  %16 = icmp eq i64 %n.vec, 16
  br i1 %16, label %middle.block, label %vector.body.4

vector.body.4:                                    ; preds = %vector.body.3
  %next.gep.4 = getelementptr i8, ptr %data.addr.019, i64 16
  %wide.load.4 = load <4 x i8>, ptr %4, align 16
  store <4 x i8> %wide.load.4, ptr %next.gep.4, align 1
  %17 = add <4 x i32> %8, <i32 -5, i32 -5, i32 -5, i32 -5>
  br label %middle.block

middle.block:                                     ; preds = %vector.body.4, %vector.body.3, %vector.body.2, %vector.body.1, %vector.ph
  %.lcssa = phi <4 x i32> [ %9, %vector.ph ], [ %11, %vector.body.1 ], [ %13, %vector.body.2 ], [ %15, %vector.body.3 ], [ %17, %vector.body.4 ]
  %18 = call i32 @llvm.vector.reduce.add.v4i32(<4 x i32> %.lcssa)
  %cmp.n = icmp eq i64 %n.vec, %wide.trip.count
  br i1 %cmp.n, label %for.cond.cleanup, label %for.body.preheader

for.body.preheader:                               ; preds = %while.body, %middle.block
  %indvars.iv.ph = phi i64 [ 0, %while.body ], [ %n.vec, %middle.block ]
  %size.addr.116.ph = phi i32 [ %size.addr.020, %while.body ], [ %18, %middle.block ]
  %data.addr.115.ph = phi ptr [ %data.addr.019, %while.body ], [ %ind.end, %middle.block ]
  br label %for.body

for.cond.cleanup:                                 ; preds = %for.body, %middle.block
  %incdec.ptr.lcssa = phi ptr [ %ind.end, %middle.block ], [ %incdec.ptr, %for.body ]
  %dec.lcssa = phi i32 [ %18, %middle.block ], [ %dec, %for.body ]
...
for.body:                                         ; preds = %for.body.preheader, %for.body
  ...
  %size.addr.116 = phi i32 [ %dec, %for.body ], [ %size.addr.116.ph, %for.body.preheader ]
  %data.addr.115 = phi ptr [ %incdec.ptr, %for.body ], [ %data.addr.115.ph, %for.body.preheader ]
  %arrayidx = getelementptr inbounds [20 x i8], ptr %buff, i64 0, i64 %indvars.iv
  %19 = load i8, ptr %arrayidx, align 1
  %incdec.ptr = getelementptr inbounds i8, ptr %data.addr.115, i64 1
  store i8 %19, ptr %data.addr.115, align 1
  %indvars.iv.next = add nuw nsw i64 %indvars.iv, 1
  %dec = add i32 %size.addr.116, -1
  %exitcond.not = icmp eq i64 %indvars.iv.next, %wide.trip.count
  br i1 %exitcond.not, label %for.cond.cleanup, label %for.body

while.end:                                        ; preds = %for.cond.cleanup, %if.end
...
entry:
  ...
  store i8 1, ptr getelementptr inbounds (%class.CRandomGenerator, ptr @g_RandomGenerator, i64 0, i32 1), align 1
  ret void
}
