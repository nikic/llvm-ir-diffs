; ModuleID = '/home/npopov/repos/llvm-test-suite/CTMark/ClamAV/libclamav_filetypes.c'
source_filename = "/home/npopov/repos/llvm-test-suite/CTMark/ClamAV/libclamav_filetypes.c"
...
if.then15.i:                                      ; preds = %if.then.i
  ...
  %3 = load ptr, ptr %descr.i, align 8
  tail call void (ptr, ...) @cli_dbgmsg(ptr noundef nonnull @.str, ptr noundef %3) #10
...
for.inc.i:                                        ; preds = %if.then.i, %for.body.i
  ...
  %5 = load ptr, ptr %magic.i, align 8
...
if.end42:                                         ; preds = %if.then32, %if.end28
  ...
  %15 = load i32, ptr %phishing, align 4
  %and = and i32 %15, 2
...
if.end79:                                         ; preds = %if.end64
  ...
  %spec.select194 = select i1 %cmp74.not, i32 528, i32 %ret.2213
  call void @cli_ac_freedata(ptr noundef nonnull %mdata) #10
  br i1 %cmp74.not, label %cleanup84.thread204, label %while.body

cleanup84.thread204:                              ; preds = %if.end79, %if.end79.thread
  %ret.3220 = phi i32 [ %ret.2213, %if.end79.thread ], [ %spec.select194, %if.end79 ]
...
cleanup84.thread:                                 ; preds = %if.then47
...
if.end90:                                         ; preds = %cleanup84.thread204, %cleanup84.thread, %if.end16, %if.end42, %if.end
  %ret.6 = phi i32 [ %ret.1, %if.end42 ], [ %ret.0, %if.end ], [ %call20, %if.end16 ], [ %ret.1, %cleanup84.thread ], [ %ret.3220, %cleanup84.thread204 ]
  %19 = and i32 %ret.6, -2
  %or.cond143 = icmp eq i32 %19, 500
  br i1 %or.cond143, label %if.then95, label %cleanup132

if.then95:                                        ; preds = %entry, %if.end90
  %ret.6208 = phi i32 [ %ret.6, %if.end90 ], [ 501, %entry ]
...
if.end130:                                        ; preds = %if.end130.sink.split, %if.else123
  %ret.8 = phi i32 [ %ret.6208, %if.else123 ], [ %ret.8.ph, %if.end130.sink.split ]
...
cleanup132:                                       ; preds = %cleanup84, %if.end130, %if.end90, %if.then95, %if.else, %if.end12, %if.then8
  %retval.3 = phi i32 [ %ret.2213, %cleanup84 ], [ 500, %if.then8 ], [ 500, %if.end12 ], [ 500, %if.else ], [ %ret.6208, %if.then95 ], [ %ret.8, %if.end130 ], [ %ret.6, %if.end90 ]
