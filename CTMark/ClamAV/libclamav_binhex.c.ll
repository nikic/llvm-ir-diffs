; ModuleID = '/home/npopov/repos/llvm-test-suite/CTMark/ClamAV/libclamav_binhex.c'
source_filename = "/home/npopov/repos/llvm-test-suite/CTMark/ClamAV/libclamav_binhex.c"
...
if.end11:                                         ; preds = %if.end7
  ...
  %cmp12118 = icmp sgt i64 %0, 0
  br i1 %cmp12118, label %for.cond.preheader, label %while.end.thread133

while.end.thread133:                              ; preds = %if.end11
  %call44135 = tail call i32 @munmap(ptr noundef %call8, i64 noundef %0) #7
  br label %if.end47

for.cond.preheader:                               ; preds = %if.end11, %land.lhs.true32
  %buf.0121 = phi ptr [ %incdec.ptr40, %land.lhs.true32 ], [ %call8, %if.end11 ]
  %line.0120 = phi ptr [ %call19, %land.lhs.true32 ], [ null, %if.end11 ]
  %bytesleft.0119 = phi i64 [ %dec41, %land.lhs.true32 ], [ %0, %if.end11 ]
  %1 = trunc i64 %bytesleft.0119 to i32
  %scevgep = getelementptr i8, ptr %buf.0121, i64 %bytesleft.0119
  br label %land.lhs.true

land.lhs.true:                                    ; preds = %for.cond.preheader, %for.body
  %ptr.0117 = phi ptr [ %buf.0121, %for.cond.preheader ], [ %incdec.ptr, %for.body ]
  %length.0116 = phi i32 [ 0, %for.cond.preheader ], [ %inc, %for.body ]
  %bytesleft.1115 = phi i64 [ %bytesleft.0119, %for.cond.preheader ], [ %dec, %for.body ]
  %2 = load i8, ptr %ptr.0117, align 1
...
for.body:                                         ; preds = %land.lhs.true
  %inc = add nuw nsw i32 %length.0116, 1
  %dec = add nsw i64 %bytesleft.1115, -1
  %incdec.ptr = getelementptr inbounds i8, ptr %ptr.0117, i64 1
  %tobool.not = icmp eq i64 %dec, 0
  br i1 %tobool.not, label %for.end, label %land.lhs.true

for.end:                                          ; preds = %land.lhs.true, %land.lhs.true, %for.body
  %bytesleft.1.lcssa = phi i64 [ %bytesleft.1115, %land.lhs.true ], [ %bytesleft.1115, %land.lhs.true ], [ 0, %for.body ]
  %length.0.lcssa = phi i32 [ %length.0116, %land.lhs.true ], [ %length.0116, %land.lhs.true ], [ %1, %for.body ]
  %ptr.0.lcssa = phi ptr [ %ptr.0117, %land.lhs.true ], [ %ptr.0117, %land.lhs.true ], [ %scevgep, %for.body ]
  %add = add nuw nsw i32 %length.0.lcssa, 1
  %conv18 = zext i32 %add to i64
  %call19 = tail call ptr @cli_realloc(ptr noundef %line.0120, i64 noundef %conv18) #7
  %cmp20 = icmp eq ptr %call19, null
  br i1 %cmp20, label %while.end, label %if.end23

if.end23:                                         ; preds = %for.end
  %conv24 = zext i32 %length.0.lcssa to i64
  tail call void @llvm.memcpy.p0.p0.i64(ptr nonnull align 1 %call19, ptr align 1 %buf.0121, i64 %conv24, i1 false)
...
land.lhs.true32:                                  ; preds = %if.end23
  %3 = load i8, ptr %ptr.0.lcssa, align 1
  ...
  %ptr.1 = getelementptr inbounds i8, ptr %ptr.0.lcssa, i64 %ptr.1.idx
...
while.end:                                        ; preds = %for.end
  ...
  %tobool45.not = icmp eq ptr %line.0120, null
  br i1 %tobool45.not, label %if.end47, label %if.then46

if.then46.sink.split:                             ; preds = %land.lhs.true32, %if.end23
  %call44131 = tail call i32 @munmap(ptr noundef %call8, i64 noundef %0) #7
  br label %if.then46

if.then46:                                        ; preds = %if.then46.sink.split, %while.end
  %line.2113 = phi ptr [ %line.0120, %while.end ], [ %call19, %if.then46.sink.split ]
  tail call void @free(ptr noundef nonnull %line.2113) #7
  br label %if.end47

if.end47:                                         ; preds = %while.end.thread133, %if.then46, %while.end
